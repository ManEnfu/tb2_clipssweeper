...
(defrule check-safes
    (declare (salience 30))
    (phase check-closed)
    (tile-safe (row ?r) (col ?c))
    (not (tile-open (row ?r) (col ?c)))
    =>
    (assert (tile-closed-check (row ?r) (col ?c) (status safe)))
    (do-for-all-facts ((?ta tile-adjacent))
        (and 
            (= ?ta:row1 ?r) (= ?ta:col1 ?c)
            (any-factp ((?to tile-open)) (and (= ?ta:row2 ?to:row) (= ?ta:col2 ?to:col)))
        )
        (assert (tile-inc (row ?ta:row2) (col ?ta:col2) (id (+ (* 100 ?r) ?c)) (type safe)))
    )
)
...
(defrule immediate-act
    (declare (salience 24))
    ?f <- (phase check-closed)
    (tile-closed-check (row ?r) (col ?c) (status safe|flag))
    =>
    (retract ?f)
    (assert (phase select-action))
)
